...
describe('getImportedPackagesFromFiles', () => {
  test('Package 내의 import package들을 정상적으로 가져와야 한다', async () => {
    ...
    const aPackages: Set<string> = await getImportedPackagesFromFiles(await getSources(path.join(cwd, 'src/_packages/a')));
    expect(Array.from(aPackages).sort()).toStrictEqual(['c', 'react', 'rxjs']);
    
    const bPackages: Set<string> = await getImportedPackagesFromFiles(await getSources(path.join(cwd, 'src/_packages/b')));
    expect(Array.from(bPackages).sort()).toStrictEqual(['moment']);
    
    const cPackages: Set<string> = await getImportedPackagesFromFiles(await getSources(path.join(cwd, 'src/_packages/c')));
