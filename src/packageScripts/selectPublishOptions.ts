...
export function getVersions({currentPackageJson, remotePackageJson}: PackagePublishOption): {currentVersion: string, remoteVersion: string | undefined} {
...
export async function selectPublishOptions({publishOptions, choice}: {publishOptions: PackagePublishOption[], choice: boolean}): Promise<PackagePublishOption[]> {
  if (!choice) {
