...
} from '@rocket-scripts/browserslist';
...
describe('browserslist', () => {
  test('should get a correct query by BROWSERSLIST_ENV', () => {
    ...
    expect(getBrowserslistQuery({ cwd, env: 'production' })).toBe(
      defaultQuery.production,
    );
    expect(getBrowserslistQuery({ cwd, env: 'development' })).toBe(
      defaultQuery.development,
    );
    expect(getBrowserslistQuery({ cwd, env: 'server' })).toBe(
      defaultQuery.server,
    );
    expect(getBrowserslistQuery({ cwd, env: 'server_development' })).toBe(
      defaultQuery.server_development,
    );
    expect(getBrowserslistQuery({ cwd, env: 'electron' })).toBe(
      defaultQuery.electron,
    );
    expect(getBrowserslistQuery({ cwd, env: 'package' })).toBe(
  ...
  test('should get a correct query by package.json', () => {
    const cwd: string = path.join(
      process.cwd(),
      'test/fixtures/browserslist/custom',
    );
    ...
    expect(getBrowserslistQuery({ cwd, env: 'server_development' })).toBe(
      browserslist.server_development,
    );
    expect(getBrowserslistQuery({ cwd, env: 'electron' })).toBe(
  ...
  test('should get default query by wrong env', () => {
    ...
    // @ts-expect-error
    expect(getBrowserslistQuery({ cwd, env: '???' })).toBe(
