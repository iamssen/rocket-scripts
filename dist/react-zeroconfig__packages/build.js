...
const browserslist_1 = require("@react-zeroconfig/browserslist");
...
const getIndexFile_1 = require("./entry/getIndexFile");
...
const fsPackagesCopyFilter_1 = require("./static-files/fsPackagesCopyFilter");
async function build({ cwd, outDir: _outDir, tsconfig = 'tsconfig.json', mode = 'production' }) {
    const outDir = rule_1.icuFormat(_outDir, { cwd, mode });
    // ---------------------------------------------
    // rule
    ...
    const targets = browserslist_1.getBrowserslistQuery({ cwd, env: 'package' });
    const externals = [];
    ...
    for (const packageName of order) {
        ...
        const outputDir = path_1.default.join(outDir, flat_package_name_1.flatPackageName(packageName));
        // ---------------------------------------------
        ...
        // copy files
        // ---------------------------------------------
        await fs_extra_1.default.copy(sourceDir, outputDir, { filter: fsPackagesCopyFilter_1.fsPackagesCopyFilter });
        // ---------------------------------------------
        // webpack build
        // ---------------------------------------------
        const babelConfig = {
