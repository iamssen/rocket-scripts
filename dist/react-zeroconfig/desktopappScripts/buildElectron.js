...
async function buildElectron({ cwd, app, zeroconfigPath, staticFileDirectories, output, extend }) {
  ...
  const webpackMainConfig = webpack_merge_1.default(
  ...
  const webpackRendererConfig = webpack_merge_1.default(
    createWebpackBaseConfig_1.createWebpackBaseConfig({ zeroconfigPath }),
    {
  ...
  try {
    ...
    console.log(
      chalk_1.default.bold.yellow(
        `cd ${path_1.default.relative(
          cwd,
          path_1.default.join(output, 'electron'),
