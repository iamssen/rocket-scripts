...
exports.findInternalPackageMissingDependencies = void 0;
...
const node_api_list_json_1 = __importDefault(require("./node-api-list.json"));
async function findInternalPackageMissingDependencies({ packageDir, }) {
    ...
    for (const packageName of importedPackages) {
        if (wrotePackages.has(packageName) || node_api_list_json_1.default.indexOf(packageName) > -1) {
            importedPackages.delete(packageName);
        }
    }
    return importedPackages.size > 0 ? importedPackages : undefined;
}
