...
module.exports = function ({ buildOption, appDirectory, zeroconfigDirectory }) {
    if (!buildOption.declaration)
        return Promise.resolve();
    return new Promise((resolve, reject) => {
        const { jsx, experimentalDecorators, downlevelIteration, importHelpers, resolveJsonModule, allowSyntheticDefaultImports, esModuleInterop, alwaysStrict, strictNullChecks, strictBindCallApply, strictFunctionTypes, strictPropertyInitialization, lib, } = getCompilerOptions(path_1.default.join(appDirectory, 'tsconfig.json'));
        const { jsx: jsxDefault, experimentalDecorators: experimentalDecoratorsDefault, downlevelIteration: downlevelIterationDefault, importHelpers: importHelpersDefault, resolveJsonModule: resolveJsonModuleDefault, allowSyntheticDefaultImports: allowSyntheticDefaultImportsDefault, esModuleInterop: esModuleInteropDefault, alwaysStrict: alwaysStrictDefault, strictNullChecks: strictNullChecksDefault, strictBindCallApply: strictBindCallApplyDefault, strictFunctionTypes: strictFunctionTypesDefault, strictPropertyInitialization: strictPropertyInitializationDefault, lib: libDefault, } = getCompilerOptions(path_1.default.join(zeroconfigDirectory, 'configs/tsconfig.json'));
        const program = typescript_1.default.createProgram([buildOption.file], {
            // language setting
            jsx: replaceUndefined(jsx, jsxDefault) === 'react' ? typescript_1.JsxEmit.React : typescript_1.JsxEmit.None,
