...
async function getStaticFileDirectories({ staticFileDirectories, staticFilePackages, cwd }) {
    ...
    if (typeof staticFileDirectories === 'string') {
        const manualDirectories = staticFileDirectories.split(' ')
        ...
        const internalPackageDirectories = await getInternalPackagePublicDrectories_1.getInternalPackagePublicDrectories({ packageDir: path_1.default.join(cwd, 'src/_packages') });
        directories.push(...internalPackageDirectories);
    }
    if (typeof staticFilePackages === 'string') {
        const packageDirectories = staticFilePackages.split(' ')
            .map(packageName => {
            const paths = [
