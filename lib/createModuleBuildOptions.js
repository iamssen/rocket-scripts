...
module.exports = function ({ modules, appDirectory }) {
    const packageJsonFiles = Object.keys(modules).map(name => {
        ...
        return fs_extra_1.default.readJsonSync(path_1.default.join(appDirectory, `src/_modules/${groupDirectory}${name}`));
    });
    ...
    const buildOptions = sortedModuleNames.map(name => {
        ...
        const indexFile = fs_extra_1.default.existsSync(path_1.default.join(appDirectory, `src/_modules/${groupDirectory}${name}/index.tsx`))
            ? 'index.tsx'
            : 'index.ts';
        const file = path_1.default.join(appDirectory, `src/_modules/${groupDirectory}${name}/${indexFile}`);
        const buildOption = {
            name,
            group,
            groupDirectory,
