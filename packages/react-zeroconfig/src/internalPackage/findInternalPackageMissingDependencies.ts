...
export async function findInternalPackageMissingDependencies({
  packageDir,
}: {
  packageDir: string;
  ...
  const { dependencies, devDependencies, peerDependencies, optionalDependencies }: PackageJson = await fs.readJson(
    packageJson,
  );
