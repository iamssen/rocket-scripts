...
export async function getStaticFileDirectories({
  ...
  if (typeof staticFileDirectories === 'string') {
    ...
    const internalPackageDirectories: string[] = await getInternalPackagePublicDrectories({
      packageDir: path.join(cwd, 'src/_packages'),
    });
